...
// Chats donde el bot dejó de responder porque el cliente pidió hablar
// directamente con el barbero. Se usan en el panel /admin.
async function listHumanTakeoverChats() {
  const { rows } = await pool.query(
    `SELECT chat_id, updated_at FROM conversation_state WHERE step = 'human_takeover' ORDER BY updated_at DESC`
  );
  return rows;
}
...
module.exports = { initDb, getState, setState, clearState, listHumanTakeoverChats, saveAppointment, listAppointments };
